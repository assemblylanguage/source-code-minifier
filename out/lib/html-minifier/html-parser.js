/**
 * This file contains the source code for Erik Arvidsson's HTML Parser library,
 * used by the HTML Minifier to parse HTML entities.
 */
'use strict';
...
var singleAttrIdentifier = /([^\s"'<>/=]+)/, singleAttrAssigns = [/=/], singleAttrValues = [
...
})(), startTagOpen = new RegExp('^<' + qnameCapture), startTagClose = /^\s*(\/?)>/, endTag = new RegExp('^<\\/' + qnameCapture + '[^>]*>'), doctype = /^<!DOCTYPE\s?[^>]+>/i;
